...
export default function LoginModal({ onClose, onSuccess }) {
  const { signInWithGoogle, setShowLogin } = useAuth();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // If result is undefined because it redirects on mobile, we remain in loading state until redirect occurs.
      if (result) {
        setShowLogin(false);
        onSuccess?.(result);
      }
      ...
      if (err.code === 'auth/unauthorized-domain') {
        setError('This domain is not authorized for Firebase Auth. Please add this domain or local network IP (e.g. 192.168.x.x) under Authorized Domains in the Firebase Console Settings.');
      } else if (err.code === 'auth/popup-blocked') {
        setError('The Google Sign-In popup was blocked by your browser. Please allow popups or try again.');
